...
int main(int argc, char* argv[])
{
    ...
    if (argc != 2)
    {
        fprintf (stderr, "Usage:   ParsingXML <file_name.xml>\n");
  
        exit (1);
    }

    // Handle if passed in a file name, expand if it doesn't look like a URL.
    // GetFullPathName(argv[1], MAX_PATH, pszFullPath, NULL);
	// strcpy (szAppName, argv[1]);

	//Call the parsing nodes function:
	XMLDOMParsingNodes(argv[1]);
	//XMLDOMParsingNodes(szAppName);
