...
bool CIdXmlUtility::Open()
{
	if( m_strXmlFilePath == "" )
	{
		g_tLog.WriteLogWithoutTimeStamp("XML file not specified!!");
		m_status = false;
		return false;
	}
	CreateDOM();
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("XML Load Error:<%s> !!", m_strXmlFilePath.c_str());
		m_status = false;
		return false;	//GURU: returning error
	}
	g_tLog.WriteLogWithoutTimeStamp("XML Load Success:<%s>", m_strXmlFilePath.c_str());
	m_status = true;
...
bool CIdXmlUtility::OpenXMLFile(const char* szXMLFilePath)
{
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("XML Load Error:<%s> !!", szXMLFilePath);
		m_status = false;
		return false;
	}
	g_tLog.WriteLogWithoutTimeStamp("XML Load Success:<%s>", szXMLFilePath);
	m_status = true;
	return true;
}
...
bool CIdXmlUtility::OpenXMLString(const char* szXMLString)
{
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("XML Load String Error !!\n%s\n", szXMLString);
		return false;
	}
...
int CIdXmlUtility::GetValue(string strElementPath,string &strValue)
{
	  ...
	  {
		string nodeName = *i;
		g_tLog.WriteLogWithoutTimeStamp("Looking for child node:%s", nodeName.c_str());

		if( ! m_domParser->FindChildElem(nodeName.c_str()))
		{
			g_tLog.WriteLogWithoutTimeStamp("Node %s not found", nodeName.c_str());
			#if defined(WIN32) && !defined(__CYGWIN__)
				g_tLog.WriteLogWithoutTimeStamp("DOM Error Status:%s", m_domParser->GetError());
			#else
				g_tLog.WriteLogWithoutTimeStamp("DOM Error Status:%s", m_domParser->GetError().c_str());
			#endif
			return EPATH;
		}
		else
...
int CIdXmlUtility::GetAttrValue(string strElementPath,string strAttrName,string &strValue)
{
	...
	for(; i != nodes.end(); i++)
    {
		string nodeName = *i;
		#if defined(WIN32) && !defined(__CYGWIN__)
			//g_tLog.WriteLogWithoutTimeStamp("Current Node:%s", m_domParser->GetTagName());
		#else
			g_tLog.WriteLogWithoutTimeStamp("Current Node:%s", m_domParser->GetTagName().c_str());
		#endif
...
int CIdXmlUtility::MoveToPath(const char* szElementPath, bool bAbsolutePath)
{
	...
	while(i != nodes.end())
    {
		...
		if(!m_domParser->FindChildElem(i->c_str()))
        {
            g_tLog.WriteLogWithoutTimeStamp("Node %s not found", i->c_str());
								...
								const char* szAttrValue, bool bAbsolutePath)
{
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("Element Path and attr name required!! Cannot move to path.");
		return EPATH;
	}
	string strElementPath = szElementPath;
	...
	else if(bAbsolutePath)
	{
		...
		{
			g_tLog.WriteLogWithoutTimeStamp("Incorrect Root Element!! Cannot move to path.");
			return EPATH;
		}
		m_strElementPath = szElementPath;
	...
	while(strNodeID != strAttrValue)
	{
		...
		{
			g_tLog.WriteLogWithoutTimeStamp("No <%s> element with matching attribute value <%s> for <%s>",
...
int CIdXmlUtility::MoveToNextAttrPath(const char* szAttrName,const char* szAttrValue)
{
	...
	while(strNodeID != szAttrValue)
	{
		...
		{
			g_tLog.WriteLogWithoutTimeStamp("No element with matching attribute value <%s> for <%s>",
							...
							const char* szAttrValue2, bool bAbsolutePath)
{
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("Element Path and attr names required!! Cannot move to path.");
		return EPATH;
	}
	string strElementPath = szElementPath;
	int nLastElemIndex	  = strElementPath.find_last_of("/");
	...
	while((strNodeAttrValue1 != strAttrValue1) || (strNodeAttrValue2 != strAttrValue2))
	{
		if(m_domParser->FindChildElem(strLastElem.c_str()) == 0)
		{
			g_tLog.WriteLogWithoutTimeStamp("No <%s> element with matching values for <%s> and <%s>",
...
int CIdXmlUtility::GetCurrTagAttr(const char* szAttrName, string& strAttrValue)
{
	...
	{
//		g_tLog.WriteLogWithoutTimeStamp("Attribute <%s> not found for <%s>",
...
int CIdXmlUtility::GetCurrTagChild(const char* szChildTagName, string& strChildTagValue)
{
	...
	{
		g_tLog.WriteLogWithoutTimeStamp("Child <%s> not found under <%s>",
		   					...
		   					const char* szAttrValue, string& strChildTagValue)
{
	...
	while(strNodeAttrValue != strAttrValue)
	{
		...
		{
			g_tLog.WriteLogWithoutTimeStamp("No <%s> element with matching attribute <%s>",
								 ...
								 string& strChildTagValue, string& strChildAttr1Value)
{
	if(!m_domParser->FindChildElem(szChildTagName))
	{
		g_tLog.WriteLogWithoutTimeStamp("No more child <%s> under <%s>",
									szChildTagName, m_strElementPath.c_str());
		return EPATH;
	}
	strChildTagValue = m_domParser->GetChildData();
...
bool CIdXmlUtility::AddSubDoc(string strElementPath,string strInputSubDoc,bool bClear)
{
	...
	//g_tLog.WriteLogWithoutTimeStamp("strInputSubDoc<%s>",strInputSubDoc.c_str());
	CIdXmlUtility tXml;
	if(!tXml.OpenXMLString(strInputSubDoc.c_str()))
	{
		g_tLog.WriteLogWithoutTimeStamp("XML open error !!!<%s>",strInputSubDoc.c_str());
		return false;
	}
	...
	while(tXml.FindChildElem())
	{
		...
		if(bClear && strTags.find(strChildTagName) == string::npos)
		{
			...
			g_tLog.WriteLogWithoutTimeStamp("***strChildTagName<%s>",strChildTagName.c_str());
